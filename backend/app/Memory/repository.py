...
def list_entities(session: Session, user_id: Optional[int] = None) -> List[MemoryEntityModel]:
    stmt = select(MemoryEntityModel)
    if user_id is not None:
        stmt = stmt.where(MemoryEntityModel.user_id == user_id)
    return list(session.scalars(stmt.order_by(MemoryEntityModel.id.asc())).all())
