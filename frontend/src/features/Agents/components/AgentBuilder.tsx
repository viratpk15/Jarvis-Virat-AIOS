...
import { Save, Bot, Cpu, Wrench, Sparkles, CheckCircle } from "lucide-react"
...
export default function AgentBuilder() {
  ...
  const handleSave = (e: React.FormEvent) => {
    e.preventDefault()
    setSavedStatus(`Agent '${name}' configured and saved to registry!`)
  ...
  return (
    <div className="space-y-4 font-mono text-xs max-w-4xl">
      <div className="flex items-center justify-between border-b border-border/40 pb-3">
        ...
        <button
          onClick={handleSave}
          className="flex items-center gap-1.5 px-3 py-1.5 font-bold rounded-lg bg-cyan-500/20 text-cyan-300 border border-cyan-500/40 hover:bg-cyan-500/30 cursor-pointer transition-all"
        >
